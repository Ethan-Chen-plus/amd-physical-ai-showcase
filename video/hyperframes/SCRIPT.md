...
- A successful or especially informative task may keep its full trajectory. Long source clips are trimmed to the most informative motion.
...
3. RoboCasa365 gives the project a household manipulation benchmark with fixed tasks, seeds, task predicates, and four-view video.
...
6. Each result carries its own protocol and denominator: official seeds set the benchmark, success-seed archives provide additional task traces, and stage records explain behavior.
...
12. PAC-MAN predictive control tracks a projectile around Unitree G1, predicts its future path, and redirects motion to preserve clearance on AMD. A compact boundary sequence then maps navigation, contact, release, and recovery to stage-level evidence.
