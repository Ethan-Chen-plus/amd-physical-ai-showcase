...
QA="$DIST/qa-english-first-10s.mp4"

ffmpeg -hide_banner -loglevel error -y \
  -i "$SOURCE" -i "$ROOT/audio/generated/amd-physical-ai-demo-audio.wav" -t 10 \
  -vf "subtitles=$ROOT/subtitles/en.srt:charenc=UTF-8:force_style='$STYLE'" \
...
ffmpeg -hide_banner -loglevel error -y -ss 1.5 -i "$QA" \
  -frames:v 1 "$DIST/qa-english-frame-01.png"
ffmpeg -hide_banner -loglevel error -y -ss 5.0 -i "$QA" \
  -frames:v 1 "$DIST/qa-english-frame-02.png"
