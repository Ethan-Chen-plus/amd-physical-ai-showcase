...
`narration/*.txt` holds the English chapter script. `render_audio.sh` renders each chapter with the Kokoro `af_heart` English voice, places the chapters on the 260-second timeline, creates a restrained original score with FFmpeg, and mixes the two tracks into the WAV consumed by `index.html`.
